...
gerar_agenda_temporal(SeqMotoristas,SeqWorkBlockIds,Agenda,Resultado):-
	...
	(
		% Se o motorista já tiver workblocks na agenda, faz append deste workblock ao final da 
        %lista de workblocks desse motorista
		(
...
concatenar_blocos_consecutivos(ListaWorkBlock,ListaWorkBlockConcatenada):-
	...
	% Se não forem iguais, então passa uma sublista sem o primeiro workblock, e depois faz append do 
    %primeiro work block ao resultado
...
avalia_agenda(Agenda,V):-
    ...
    restricao_horario_contrato(Motorista,ListaWorkBlock,Resultado5),
    Resultado is Resultado1 + Resultado2 + Resultado3 + Resultado4 + Resultado5,
